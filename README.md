...
The repository has a checked-in Vitest suite (Node unit tests plus serial PostgreSQL integration tests), a consolidated phase evidence gate, and GitHub Actions CI. Successful `main` verification can trigger the checked-in Coolify Docker deployment workflow. Coverage tooling and automated end-to-end browser tests remain absent. UI actions should be treated as demonstrations unless their behavior is explicitly backed by a route handler and Prisma service.
