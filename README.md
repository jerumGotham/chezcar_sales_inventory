...
- **Reports** — both sales reports and Returns & Warranty default to the complete current Manila month. Sales by Salesperson shares Sales filters and verified-sale query, with historical attribution options, stable-ID totals, and grouped receipts/PDF; existing Sales totals and percentages are unchanged. Compact report buttons wrap side by side; filter edits remain pending until Apply Filters. Inventory Summary shows current branch-only available stock, excluding Stock Room. Inventory Movements and Low Stock report types remain deferred. This 2026-09-08 revision is source-only, with no tests/build or runtime verification; Sales by Salesperson needs no new migration and does not resolve existing pending migrations. See `docs/product/REPORTS-SPEC.md`.
