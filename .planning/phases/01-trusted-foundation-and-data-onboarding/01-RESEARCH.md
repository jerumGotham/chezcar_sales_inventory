...
| Role-aware navigation and scope feedback | Frontend Server | Browser / Client | A persisted browser-safe shell DTO drives filtered navigation and global AppHeader scope labels. Accounting displays `Business-wide` globally with null location and no Inventory capability; Inventory selectors exist only for authorized Admin/Stock/Branch roles. [VERIFIED: lib/menu.ts:17-33] |
