...
const TRANSFER_INCLUDE = {
  ...
  discrepancy: { include: { lines: true, reportedBy: { select: { name: true, role: true } } } },
  investigation: { include: { submittedBy: { select: { name: true, role: true } } } },
  resolution: { include: { lines: true, postedBy: { select: { name: true, role: true } } } },
...
type TransferRecord = Prisma.StockTransferGetPayload<{ include: typeof TRANSFER_INCLUDE }>;
...
async function notifyUsersForTransfer(
  tx: Prisma.TransactionClient,
  transfer: { id: string; reference: string; destinationId: string; destination: { name: string } },
  recipients: Array<{ role: "ADMIN" | "STOCK_STAFF" | "BRANCH_STAFF"; locationId?: string }>,
  notification: { title: string; description: string; type: "INFO" | "WARNING" | "SUCCESS" },
) {
  const users = await Promise.all(recipients.map((recipient) => activeUsersForRole(tx, recipient.role, recipient.locationId)));
...
export async function listTransfers(actor: AuthContext) {
  ...
  return transfers.map((transfer) => serializeTransfer(transfer, actor.role === "ADMIN"));
}
...
export async function finalizeTransfer(actor: AuthContext, id: string, version: number) {
  ...
  return prisma.$transaction(async (tx) => {
    const transfer = await lockTransfer(tx, id);
    if (transfer.status === "FOR_DISPATCH") return serializeTransfer(transfer, actor.role === "ADMIN");
    assertVersion(transfer.version, version);
    ...
    await notifyUsersForTransfer(tx, updated, [{ role: "STOCK_STAFF", locationId: await stockRoomId(tx) }], {
...
export async function dispatchTransfer(actor: AuthContext, id: string, version: number) {
  ...
  return prisma.$transaction(async (tx) => {
    const transfer = await lockTransfer(tx, id);
    if (transfer.status === "IN_TRANSIT") return serializeTransfer(transfer, actor.role === "ADMIN");
    assertVersion(transfer.version, version);
    ...
    await notifyUsersForTransfer(tx, updated, [{ role: "BRANCH_STAFF", locationId: updated.destinationId }], {
...
export async function confirmReceipt(actor: AuthContext, id: string, version: number) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (transfer.status === "RECEIVED") return serializeTransfer(transfer, actor.role === "ADMIN");
    assertVersion(transfer.version, version);
    ...
    const updated = await tx.stockTransfer.update({ where: { id }, data: { status: "RECEIVED", receivedById: actor.userId, receivedAt: new Date(), version: { increment: 1 } }, include: TRANSFER_INCLUDE });
    await notifyUsersForTransfer(tx, updated, [{ role: "STOCK_STAFF", locationId: await stockRoomId(tx) }], {
...
export async function reportDiscrepancy(actor: AuthContext, id: string, input: DiscrepancyInput) {
  ...
  return prisma.$transaction(async (tx) => {
    const transfer = await lockTransfer(tx, id); ensureBranchScope(actor, transfer.destinationId);
    if (transfer.status === "DISCREPANCY_REPORTED") return serializeTransfer(transfer, actor.role === "ADMIN");
    ...
    const updated = await tx.stockTransfer.update({ where: { id }, data: { status: "DISCREPANCY_REPORTED", version: { increment: 1 } }, include: TRANSFER_INCLUDE });
    await notifyUsersForTransfer(tx, updated, [{ role: "STOCK_STAFF", locationId: await stockRoomId(tx) }], {
...
export async function submitInvestigation(actor: AuthContext, id: string, input: InvestigationInput) {
  assertStockStaff(actor);
  return prisma.$transaction(async (tx) => { const transfer = await lockTransfer(tx, id); if (transfer.status === "UNDER_REVIEW") return serializeTransfer(transfer, actor.role === "ADMIN"); assertVersion(transfer.version, input.version); if (transfer.status !== "DISCREPANCY_REPORTED") throw new TransferError("INVALID_STATE", "Only reported discrepancies can be investigated"); await tx.stockTransferInvestigation.create({ data: { transferId: id, submittedById: actor.userId, findings: input.findings } }); const updated = await tx.stockTransfer.update({ where: { id }, data: { status: "UNDER_REVIEW", version: { increment: 1 } }, include: TRANSFER_INCLUDE }); await notifyUsersForTransfer(tx, updated, [{ role: "ADMIN" }], { title: "Discrepancy ready for approval", description: `${updated.reference} has Stock Staff investigation and needs final Admin resolution.`, type: "WARNING" }); return serializeTransfer(updated, actor.role === "ADMIN"); }, { isolationLevel: "Serializable" });
}
...
export async function resolveTransfer(actor: AuthContext, id: string, input: ResolutionInput) {
  ...
  return prisma.$transaction(async (tx) => {
    const transfer = await lockTransfer(tx, id);
    if (transfer.status === "RESOLVED") return serializeTransfer(transfer, actor.role === "ADMIN");
    assertVersion(transfer.version, input.version);
    ...
    await tx.stockTransferResolution.create({ data: { transferId: id, postedById: actor.userId, notes: input.notes, lines: { create: input.lines.map((line) => ({ transferLineId: line.lineId, destinationQty: line.destinationQuantity, restoreToSrQty: line.restoreToSrQuantity, lossQty: line.lossQuantity })) } } });
    ...
    const updated = await tx.stockTransfer.update({ where: { id }, data: { status: "RESOLVED", version: { increment: 1 } }, include: TRANSFER_INCLUDE });
    await notifyUsersForTransfer(tx, updated, [{ role: "ADMIN" }, { role: "BRANCH_STAFF", locationId: updated.destinationId }], {
...
function actorLabel(actor: { name: string; role: string } | null) {
  return actor ? `${actor.name} (${actor.role.replaceAll("_", " ")})` : "System";
}

function serializeTransfer(transfer: TransferRecord, includeAudit = false) {
  const timeline = includeAudit ? [
    { label: "Draft created", actor: actorLabel(transfer.createdBy), at: transfer.createdAt.toISOString() },
    transfer.finalizedAt ? { label: "Finalized for dispatch", actor: actorLabel(transfer.finalizedBy), at: transfer.finalizedAt.toISOString() } : null,
    transfer.dispatchedAt ? { label: "Dispatched from Stock Room", actor: actorLabel(transfer.dispatchedBy), at: transfer.dispatchedAt.toISOString() } : null,
    transfer.receivedAt ? { label: "Exact receipt confirmed", actor: actorLabel(transfer.receivedBy), at: transfer.receivedAt.toISOString() } : null,
    transfer.discrepancy ? { label: "Discrepancy reported", actor: actorLabel(transfer.discrepancy.reportedBy), at: transfer.discrepancy.reportedAt.toISOString(), notes: transfer.discrepancy.notes } : null,
    transfer.investigation ? { label: "Investigation submitted", actor: actorLabel(transfer.investigation.submittedBy), at: transfer.investigation.submittedAt.toISOString(), notes: transfer.investigation.findings } : null,
    transfer.resolution ? { label: "Admin resolution approved", actor: actorLabel(transfer.resolution.postedBy), at: transfer.resolution.postedAt.toISOString(), notes: transfer.resolution.notes } : null,
  ...
  return {
    ...
    lines: transfer.lines.map((line) => ({
      ...
      discrepancy: transfer.discrepancy?.lines.find((item) => item.transferLineId === line.id) ?? null,
      resolution: transfer.resolution?.lines.find((item) => item.transferLineId === line.id) ?? null,
    })),
    discrepancy: transfer.discrepancy ? { notes: transfer.discrepancy.notes, reportedAt: transfer.discrepancy.reportedAt.toISOString() } : null,
    investigation: transfer.investigation ? { findings: transfer.investigation.findings, submittedAt: transfer.investigation.submittedAt.toISOString() } : null,
    resolution: transfer.resolution ? { notes: transfer.resolution.notes, postedAt: transfer.resolution.postedAt.toISOString() } : null,
    timeline,
    movements: includeAudit ? transfer.movements.map((movement) => ({
      id: movement.id,
      type: movement.type,
