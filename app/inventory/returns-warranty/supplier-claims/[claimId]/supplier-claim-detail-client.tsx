...
import { ArrowLeft, Ban, CheckCircle2, PackageCheck, PackagePlus, Printer, Receipt, Send, Trash2, Undo2, Upload, Wrench, X, type LucideIcon } from "lucide-react";
...
import { canRunSupplierClaimAction, supplierClaimActionCapabilities, supplierClaimActionLabels, supplierClaimReasonLabels, supplierClaimStatusLabels, type SupplierClaimAction } from "@/lib/contracts/supplier-claims";
...
const SIMPLE_ACTIONS: readonly SupplierClaimAction[] = ["submit", "reject", "complete", "cancel"];
...
/**
 * Pressing an action only moved the status badge and the quantity columns, which
 * is easy to miss, so every action now says what it just did.
 */
const ACTION_RESULTS: Record<SupplierClaimAction, string> = {
  submit: "Claim submitted to the supplier.",
  reject: "Recorded that the supplier refused this claim.",
  complete: "Claim completed.",
  cancel: "Draft cancelled.",
  "return-to-supplier": "Returned to the supplier and now awaiting them",
  "send-repair": "Sent out for repair and now awaiting the supplier",
  "receive-replacement": "Replacement received into sellable stock",
  "release-repaired": "Released into sellable stock",
  "receive-repaired": "Repaired items received into sellable stock",
  writeoff: "Written off and removed from stock",
};
...
export function SupplierClaimDetailClient({ capabilities }: { capabilities: readonly string[] }) {
  ...
  const mutation = useMutation({ mutationFn: async ({ action, idempotencyKey, lines = [], targetDate }: { action: SupplierClaimAction; idempotencyKey: string; lines?: Array<{ productId: string; quantity: number }>; targetDate?: string }) => { const response = await fetch(`/api/supplier-claims/${claimId}/${action}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ version: query.data!.version, idempotencyKey, lines, targetDate }) }); const json = await response.json(); if (!response.ok) throw new Error(json.error?.message ?? "Unable to update claim"); return json.data as Claim; }, onSuccess: (data, variables) => { actionKeyRef.current = null; queryClient.setQueryData(["supplier-claim", claimId], data); const pieces = variables.lines?.reduce((sum, line) => sum + line.quantity, 0) ?? 0; setMessage(pieces ? `${ACTION_RESULTS[variables.action]}: ${pieces} ${pieces === 1 ? "piece" : "pieces"}.` : ACTION_RESULTS[variables.action]); }, onError: () => { setMessage(""); } });
  ...
  return <PageShell title={claim.reference} subtitle={`${claim.supplierName} at ${claim.locationName}`} actions={<div className="flex flex-wrap items-center gap-2"><Link className={buttonVariants({ variant: "outline" })} href="/inventory/returns-warranty/supplier-claims"><ArrowLeft aria-hidden="true" />Back to supplier claims</Link><Badge variant="outline">{supplierClaimStatusLabels[claim.status] ?? claim.status}</Badge>{capabilities.includes("supplier-claims:print") ? <Link className={buttonVariants({ variant: "outline" })} href={`/inventory/returns-warranty/supplier-claims/${claim.id}/print`} target="_blank" rel="noopener noreferrer"><Printer aria-hidden="true" />Print</Link> : null}</div>}>
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]"><Card className="min-w-0"><CardHeader><CardTitle>Claimed items</CardTitle></CardHeader><CardContent><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="py-2">Item</th><th>Issue</th><th>Claimed</th><th>Held at location</th><th>Awaiting supplier</th></tr></thead><tbody>{claim.lines.map((line) => <tr key={line.productId} className="border-b"><td className="py-3"><strong>{line.productItemCode}</strong><div className="text-muted-foreground">{line.productName}</div></td><td>{supplierClaimReasonLabels[line.reason] ?? line.reason}</td><td>{line.claimedQuantity}</td><td>{line.openQuarantinedQuantity}</td><td>{line.openMissingQuantity}</td></tr>)}</tbody></table></div><p className="mt-3 text-sm text-muted-foreground">Held items are in quarantine and cannot be sold. Awaiting supplier includes missing delivery items and items sent back or out for repair.</p><div className="mt-4 max-w-xs space-y-2"><Label htmlFor="supplier-claim-target">Follow-up date and time</Label><Input ref={targetDateRef} id="supplier-claim-target" type="datetime-local" defaultValue={claim.targetDate ? new Date(new Date(claim.targetDate).getTime() - new Date(claim.targetDate).getTimezoneOffset() * 60_000).toISOString().slice(0, 16) : ""} aria-describedby="supplier-target-help" /><p id="supplier-target-help" className="text-sm text-muted-foreground">A future date is required when submitting the draft.</p></div><div className="mt-5 space-y-5">
      <section aria-labelledby="claim-status-actions" className="rounded-xl border p-4">
        <h3 id="claim-status-actions" className="text-sm font-semibold">Where the claim stands</h3>
        <p className="mb-3 text-sm text-muted-foreground">These move the claim itself. Nothing here touches stock.</p>
        <div className="flex flex-wrap gap-2">{SIMPLE_ACTIONS.filter((action) => capabilities.includes(supplierClaimActionCapabilities[action])).map((action) => {
          const Icon = ACTION_ICONS[action];
          const unavailable = !canRunSupplierClaimAction(action, claim.status);
          return <Button key={action} type="button" size="sm" variant={ACTION_VARIANTS[action]} disabled={mutation.isPending || unavailable} title={unavailable ? `Not available while the claim is ${(supplierClaimStatusLabels[claim.status] ?? claim.status).toLowerCase()}` : undefined} onClick={() => runAction(action)}><Icon aria-hidden="true" />{supplierClaimActionLabels[action]}</Button>;
        })}</div>
      </section>
      <section aria-labelledby="claim-stock-actions" className="rounded-xl border p-4">
        <h3 id="claim-stock-actions" className="text-sm font-semibold">Where the items go</h3>
        <p className="mb-3 text-sm text-muted-foreground">These move real stock. Press one only after the goods have actually moved or the repair is confirmed. The number in brackets is how many pieces the action will cover.</p>
        <div className="flex flex-wrap gap-2">{QUANTITY_ACTIONS.filter((action) => capabilities.includes(supplierClaimActionCapabilities[action])).map((action) => {
          const Icon = ACTION_ICONS[action];
          const pieces = actionLines(action).reduce((sum, line) => sum + line.quantity, 0);
          return <Button key={action} type="button" size="sm" variant={ACTION_VARIANTS[action]} className="h-auto min-h-7 whitespace-normal text-left" disabled={mutation.isPending || actionLines(action).length === 0 || !canRunSupplierClaimAction(action, claim.status)} onClick={() => runAction(action, actionLines(action))}><Icon aria-hidden="true" /><span>{supplierClaimActionLabels[action]} ({pieces})</span></Button>;
        })}</div>
        <p className="mt-3 text-sm text-muted-foreground">Making repaired items sellable lifts the quarantine hold without moving anything out of this location.</p>
      </section>
