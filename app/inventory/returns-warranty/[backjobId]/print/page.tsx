...
import { BackjobPrintControls } from "./print-controls";
...
export default async function BackjobPrintPage({ params }: { params: Promise<{ backjobId: string }> }) {
  ...
  return <main className="mx-auto max-w-4xl bg-white p-8 text-black print:max-w-none print:p-0">
    <BackjobPrintControls backjobId={record.id} />
    <div className="mb-8 flex items-start justify-between border-b pb-5"><div><h1 className="text-2xl font-bold">Predator Backjob Service Record</h1><p>{record.reference}</p></div><div className="text-right text-sm"><strong>{record.status.replaceAll("_", " ")}</strong><p>{record.locationName} ({record.locationCode})</p></div></div>
    <section className="grid gap-4 text-sm sm:grid-cols-2"><Info label="Customer" value={record.customerName} /><Info label="Installer" value={record.installerName ?? "Not assigned"} /><Info label="Original Receipt" value={record.originalReceiptNumber ?? record.legacyReference ?? "Legacy"} /><Info label="Schedule" value={record.scheduledFor ? new Date(record.scheduledFor).toLocaleString("en-PH") : "Not scheduled"} /><Info label="Purchased Items / Legacy Description" value={record.items.map((item) => `${item.productItemCode ? `${item.productItemCode} - ` : ""}${item.productName}`).join("\n")} /><Info label="Coverage" value={record.coverage} /><div className="sm:col-span-2"><Info label="Concern" value={record.concern} /></div><div className="sm:col-span-2"><Info label="Work Performed" value={record.workPerformed ?? "Pending"} /></div></section>
