  ...
  return <PageShell title="Create warranty claim" subtitle="Record the customer's issue first. Stock changes only when you separately confirm that the item was received."><Card className="max-w-3xl"><CardContent className="pt-6">
    ...
    {options.error ? <div role="alert" className="space-y-2"><p className="text-sm text-destructive">{options.error.message}</p><Button type="button" variant="outline" onClick={() => options.refetch()}><RefreshCw aria-hidden="true" />Retry loading</Button></div> : <form className="grid gap-5 md:grid-cols-2" onSubmit={submit}><fieldset disabled={submitting || options.isLoading} className="contents">
      ...
      <div className="grid gap-2"><Label htmlFor="claimQuantity">Quantity</Label><Input id="claimQuantity" name="claimQuantity" type="number" min="1" value={claimQuantity} onChange={(event) => setClaimQuantity(event.target.value)} required /></div>
       {needsWarrantyBasis && isOwner ? <>
        <p className="rounded-md border border-primary/20 bg-primary/5 p-3 text-sm text-muted-foreground md:col-span-2">No warranty was recorded when this item was sold, so the system cannot tell whether it is still covered. Set the coverage below. Only the owner can do this.</p>
        <div className="grid gap-2"><Label htmlFor="warrantyBasisMonths">How many months is this item covered?</Label><Input id="warrantyBasisMonths" name="warrantyBasisMonths" type="number" min="1" max="1200" placeholder="12" required aria-describedby="warranty-months-help" /><p id="warranty-months-help" className="text-sm text-muted-foreground">Numbers only, counted from the day it was sold. Enter 12 for a one-year warranty. The claim is refused if the item is already past that.</p></div>
        <div className="grid gap-2"><Label htmlFor="warrantyBasisReason">Where does that coverage come from?</Label><Input id="warrantyBasisReason" name="warrantyBasisReason" placeholder="Supplier invoice says 1 year" required aria-describedby="warranty-basis-help" /><p id="warranty-basis-help" className="text-sm text-muted-foreground">Your basis for honouring it, kept on the claim record.</p></div>
      </> : null}
      ...
      {isOwner ? <div className="grid gap-2 md:col-span-2"><Label htmlFor="quantityOverrideReason">Why is the customer claiming more than they bought?</Label><Input id="quantityOverrideReason" name="quantityOverrideReason" required={Boolean(mayNeedQuantityOverride)} aria-describedby="quantity-help" /><p id="quantity-help" className="text-sm text-muted-foreground">Leave this blank in the normal case. Fill it in only when the quantity above, counting earlier claims on the same item, goes past what the customer bought. Only the owner can allow that.</p></div> : null}
