...
export function WarrantyCreateClient({ isOwner }: { isOwner: boolean }) {
  ...
  return <PageShell title="Create warranty claim" subtitle="Record the customer's issue first. Stock changes only when you separately confirm that the item was received."><Card className="max-w-3xl"><CardContent className="pt-6">
    {options.error ? <div role="alert" className="space-y-2"><p className="text-sm text-destructive">{options.error.message}</p><Button type="button" variant="outline" onClick={() => options.refetch()}><RefreshCw aria-hidden="true" />Retry loading</Button></div> : <form className="grid gap-5 md:grid-cols-2" onSubmit={submit}><fieldset disabled={submitting || options.isLoading} className="contents">
      <div className="space-y-2 md:col-span-2"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={legacy} disabled={!options.data?.legacyAllowed} aria-describedby="legacy-help" onChange={(event) => { setLegacy(event.target.checked); setError(""); }} /> Sale not recorded in this system (owner only)</label><p id="legacy-help" className="text-sm text-muted-foreground">This is a legacy claim: the owner enters the customer, product, and warranty period manually, with a reason for using a sale outside the system. For a recorded sale, leave this unchecked.</p></div>
      <div className="grid min-w-0 gap-2"><Label htmlFor="locationId">Case / intake location</Label>{legacy ? <select id="locationId" name="locationId" required aria-describedby="location-help" className="h-9 min-w-0 rounded-md border bg-background px-3"><option value="">Select location</option>{options.data?.locations.map((item) => <option key={item.id} value={item.id}>{item.code} - {item.name}</option>)}</select> : <Input id="locationId" readOnly value={saleLocation ? `${saleLocation.code} - ${saleLocation.name}` : ""} placeholder={selectedLine ? "Original sale location unavailable" : "Select a purchased item first"} aria-describedby="location-help" />}<p id="location-help" className="text-sm text-muted-foreground">{legacy ? "Choose the location handling this legacy claim." : "Automatically uses the original sale location. Creating the case does not confirm physical receipt of the item."}</p>{!legacy && selectedLine && !saleLocation ? <p role="alert" className="text-sm text-destructive">The original sale location is inactive or unavailable to you. Reload the form or ask the owner to check the location and your access.</p> : null}</div>
      ...
      {legacy ? <><div className="grid gap-2"><Label htmlFor="legacyCustomerName">Customer name</Label><Input id="legacyCustomerName" name="legacyCustomerName" required /></div><div className="grid gap-2"><Label htmlFor="productId">Product</Label><select id="productId" name="productId" required className="h-9 rounded-md border bg-background px-3"><option value="">Select product</option>{options.data?.products.map((item) => <option key={item.id} value={item.id}>{item.itemCode} - {item.name}</option>)}</select></div><div className="grid gap-2"><Label htmlFor="legacySaleReference">Original receipt or sale reference (optional)</Label><Input id="legacySaleReference" name="legacySaleReference" /></div><div className="grid gap-2"><Label htmlFor="legacyReason">Why is there no system sale?</Label><Input id="legacyReason" name="legacyReason" required /></div></> : <div className="grid min-w-0 gap-2 md:col-span-2">
        ...
        <Select<PurchasedItemOption>
          ...
          aria-describedby={selectedLine ? "sale-help affected-product" : "sale-help"}
          styles={purchasedItemStyles}
          className="min-w-0 text-sm"
        />
        <p id="sale-help" className="text-sm text-muted-foreground">Only posted customer sales verified by Accounting are listed. Each claim covers one purchased line; use the product details to distinguish items on the same receipt. The case location follows your selection automatically.</p>
        ...
        {options.data && options.data.lines.length === 0 ? <p className="text-sm text-muted-foreground">No eligible purchased items found. Check the original sale and its Accounting verification.</p> : null}
      </div>}
      {needsWarrantyBasis && isOwner ? <><div className="grid gap-2"><Label htmlFor="warrantyBasisMonths">Warranty period for this claim (months)</Label><Input id="warrantyBasisMonths" name="warrantyBasisMonths" type="number" min="1" max="1200" required /></div><div className="grid gap-2"><Label htmlFor="warrantyBasisReason">Why does this warranty period apply?</Label><Input id="warrantyBasisReason" name="warrantyBasisReason" required /></div></> : null}
