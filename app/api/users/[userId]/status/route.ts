...
import { requireUserManager, setStaffStatus, usersErrorResponse } from "@/lib/server/services/users";
...
export async function POST(request: Request, context: UserRouteContext) {
  try {
    ...
    const actor = await requireUserManager(request.headers, "users:set-status");
