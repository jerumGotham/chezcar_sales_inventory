...
import { requireUserManager, updateStaffUser, usersErrorResponse } from "@/lib/server/services/users";
...
export async function PATCH(request: Request, context: UserRouteContext) {
  try {
    ...
    const actor = await requireUserManager(request.headers, "users:update");
