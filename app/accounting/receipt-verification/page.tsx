              ...
              {isLoading ? <div className="flex items-center gap-2 py-12 text-sm text-slate-500"><Loader2 className="h-4 w-4 animate-spin" /> Loading receipts...</div> : error ? <p className="py-12 text-sm text-red-600">{(error as Error).message}</p> : filteredSales.length === 0 ? <p className="py-12 text-sm text-slate-500">No receipts match the current search.</p> : (
                <table className="w-full min-w-[680px] text-left text-sm">
                  ...
                  <tbody>{filteredSales.map((sale) => <tr key={sale.id} className="border-b last:border-0">
                    ...
                    <td className="px-3 py-4 text-right"><Button size="sm" variant="outline" onClick={() => selectSale(sale)}>{selectedId === sale.id ? "Selected" : "Review"}</Button></td>
                  </tr>)}</tbody>
                </table>
              )}
            </div>
        ...
        <Card>
          <CardContent className="p-5">
            {!selectedSale ? <div className="flex min-h-80 items-center justify-center text-center text-sm text-slate-500">Select a receipt to compare its line items and manual receipt evidence.</div> : <div className="space-y-5">
              ...
              {selectedSale.receiptPhotoUrl && <Image src={selectedSale.receiptPhotoUrl} alt="Attached manual receipt" width={800} height={600} unoptimized className="max-h-64 w-full rounded-xl border object-contain" />}
              {canEditComparison && selectedSale.reviewStatus !== "VERIFIED" ? <div className="space-y-4 border-t pt-4">
                ...
                <div className="grid gap-2"><Label htmlFor="receipt-photo">Receipt photo (optional)</Label><Input id="receipt-photo" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => handlePhoto(event.target.files?.[0])} /><p className="text-xs text-slate-500">JPEG, PNG, or WebP up to 6 MB.</p></div>
                {photoPreview && <Image src={photoPreview} alt="Receipt preview" width={800} height={600} unoptimized className="max-h-40 w-full rounded-xl border object-contain" />}
                ...
                {canResolve && selectedSale.reviewStatus === "MISMATCH_REPORTED" && <><div className="grid gap-2 border-t pt-4"><Label htmlFor="resolution-note">Resolution note</Label><Textarea id="resolution-note" value={resolutionNote} onChange={(event) => setResolutionNote(event.target.value)} placeholder="Explain why this mismatch is being resolved" /></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => resolveMutation.mutate("CONFIRMED_CORRECT")} disabled={resolveMutation.isPending} className="border-sky-200 text-sky-700">Confirm correct</Button><Button onClick={() => resolveMutation.mutate("VOIDED_REPLACED")} disabled={resolveMutation.isPending || differences.length === 0} className="bg-rose-600 hover:bg-rose-700"><Upload className="mr-2 h-4 w-4" />Void and replace</Button></div></>}
                {formError && <p className="text-sm text-red-600">{formError}</p>}
              </div> : selectedSale.reviewNotes ? <div className="rounded-xl bg-rose-50 p-3 text-sm text-rose-800"><p className="font-medium">{selectedSale.mismatchCategory?.replaceAll("_", " ")}</p><p className="mt-1">{selectedSale.reviewNotes}</p></div> : null}
            </div>}
          </CardContent>
        </Card>
      </div>
