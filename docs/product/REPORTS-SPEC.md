...
- PDF metadata includes Predator company text (no literal logo placeholder or embedded logo), report title, selected filters/date range and date basis, generated by display identity, and generated at.
...
| --- | --- | --- |
| Sales | `dateFrom`, `dateTo`, `locationId`, `salespersonId`, `source`, `paymentMethod`, `dateBasis` | `collectedAt` by default, `verifiedAt` when `dateBasis=VERIFIED_DATE`; current Manila month by default |
...
Both sales reports execute the same `Payment` ledger query: effective active locations, ledger `status=ACTIVE`, review `VERIFIED`, and `verifiedAt` within the applied date range. The optional Salesperson ID, source (`DIRECT_SALE` selects the direct-sale kind; `CUSTOMER_ORDER` selects the three order kinds), and payment method narrow that query. Receipt rows include nullable `salespersonId` and the stored `salespersonName` snapshot (display fallback `Unassigned`), separately from the person who collected the payment. Rows sort by verification timestamp descending, then payment ID descending. Grand totals, branch totals, and branch percentage calculations are unchanged.

`dateBasis` chooses which date the period is measured on. `SALE_DATE`, the default, measures on the day the receipt was issued, so a printed month stops changing while Accounting works through its queue; `VERIFIED_DATE` measures on the confirmation date, which is what Accounting reconciles against. Either way only verified receipts are counted, every row shows both dates, and the report carries a `pending` count and amount for receipts issued inside the period that are still unverified. The screen shows that as a banner and the PDF prints it as a line, so a printout never understates without saying so.

Stock Movement reads `InventoryBalance` for stock and `SaleLine` joined to posted, non-voided `Sale` rows for units sold, per product per authorized location, over the applied period. Its grade is a selling pace: average days between sales is the period length divided by units sold, Fast at `FAST_MOVING_DAYS_PER_SALE` days or better, Slow beyond it, and NO MOVEMENT when nothing sold. Rows sort slowest first, and the PDF carries blank Actual and Variance columns for a physical count.

One row is one verified receipt, counted on the day it entered the period. A released order therefore appears as its downpayment, each later payment, and the balance settled at release, summing to the order total exactly once; nothing is counted twice. An order cancelled after a verified downpayment contributes only that forfeited amount, since no release receipt exists. The `Source` column names the receipt type: `Direct Sale`, `Order Downpayment`, `Order Payment`, or `Order Release`. Units and discounts belong to the receipt that completed the sale, because a downpayment moves money rather than stock; a downpayment row shows zero units and zero discount.
