...
`POST /api/customer-orders/:orderId/payment` accepts `{ amount, reference }` for an active order. Branch Staff are limited to their assigned branch; Admin may record any branch payment. The positive amount cannot exceed the remaining balance, the reference must be unique within the branch receipt ledger, and the transaction atomically adds to `downpaymentAmount`, subtracts from `remainingBalance`, and retains the first payment reference as the order's downpayment receipt. A no-DP reservation becomes a DP reservation after its first payment; later payments accumulate without releasing the order.
