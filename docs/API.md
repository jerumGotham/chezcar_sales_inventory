...
Uploading receipt evidence runs free local Tesseract OCR and persists a draft containing the recognized text, confidence, detected receipt number/total, and per-line match signals. OCR never changes the Accounting review status and never verifies or corrects a sale. A failed OCR attempt preserves the private image for manual review. Authorized Branch or Accounting users may delete evidence only while the posted sale is still unverified; deletion clears the image metadata, OCR draft/status, and reminder timestamps. Receipt-list rows expose only the persisted draft/status needed by the side-by-side Accounting UI.
...
`GET /api/stock-transfers/options` requires create or update capability plus effective access to active canonical `SR`. It returns only active products with positive server-derived availability (`onHand - reserved`) and never exposes the complete catalog to destination-only users. Draft create and update revalidate the same active-product and available-quantity rules on the server. `/stock-transfers/:transferId/print` renders a read-only checklist at every transfer status; pre-dispatch records show requested quantities and no dispatch time, while later records retain requested and dispatched quantities. The page requires transfer view capability and effective source or assigned-destination access. Printing has no inventory or workflow effect.
