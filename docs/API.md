...
| `GET` | `/api/audit?category=&search=&dateFrom=&dateTo=&page=&pageSize=` | Merged read-only activity: AuditLog rows (sign-in, failed sign-in, sign-out, master-data changes, order payments) plus derived events from inventory movements, supplier receipts, sales and their line items, sale corrections, accounting reviews and receipt photos, customer orders, stock transfer steps, discrepancies, investigations and resolutions, backjobs, warranties and supplier claims. Entries carry optional `items` and `facts` for a details view | `audit:view` |
...
| `PATCH` | `/api/sales/:saleId/salesperson` | Prisma Sale/Payment/SaleSalespersonEvent/AuditLog | `sales:salesperson:update` plus effective location access; posted sales only |
...
| `GET` | `/api/accounting/payments?page=1&pageSize=10&reviewStatus=all` | Prisma Payment/CustomerOrder/Location | `sales:verify:view`; the pre-sale receipt queue (order downpayments and later order payments), location-scoped, with the same pagination and summary counts as the sale queue |
| `POST` | `/api/accounting/payments/:paymentId/review` | Prisma Payment/Notification | Both `sales:verify` and `sales:evidence:view`; the receipt photo is required before verifying or reporting a mismatch |
| `POST` | `/api/accounting/payments/:paymentId/branch-response` | Prisma Payment/Notification | `sales:mismatch:respond`; required before a mismatch can be resolved |
| `POST` | `/api/accounting/payments/:paymentId/resolve` | Prisma Payment/CustomerOrder | `sales:resolve` to confirm as recorded; `sales:void-replace` to void, which returns the amount to the order balance |
| `POST`, `GET`, `DELETE` | `/api/accounting/payments/:paymentId/photo` | Private receipt-evidence storage + Prisma Payment/Notification | `sales:evidence:upload` / `sales:evidence:view` / `sales:evidence:delete`; a sale-linked ledger row is rejected because its photo belongs to the sale |
...
| `GET` | `/api/reports` | Prisma sales/branch-availability/movement/returns datasets | The grant named after the requested `type`, such as `reports:stock-movement`; every dataset is restricted to effective locations, and `format=pdf` needs nothing more than that same grant |
| `GET` | `/api/reports/options` | Authorized report locations and dependent filter options only | The grant named after the requested `type`; same report-specific query validation and effective scope, without fetching report rows or totals |
...
`POST /api/sales` accepts an optional `soldAt` calendar day (`YYYY-MM-DD`) for a receipt the branch is encoding late. It cannot be in the future and cannot be more than `MAX_BACKDATED_SALE_DAYS` (365) days back; an impossible day such as `2026-02-31` is rejected rather than rolled over. The value is stored on `Sale.soldAt` and copied to the sale's `Payment.collectedAt`, which is what the Sales and Sales by Salesperson reports read, so a late entry counts on the day the goods changed hands rather than the day it was typed in. Omitting it keeps the current time, and `Sale.postedAt` continues to record the encoding moment separately.

`PATCH /api/sales/:saleId/salesperson` re-credits a posted sale that named the wrong salesperson. It requires `sales:salesperson:update`, location access to the selling branch, a `POSTED` sale, and an active salesperson inside the caller's authorized locations. Money, receipt identity, lines, and inventory movements are untouched: only the salesperson snapshot changes, on both the sale and its payment ledger row, so the salesperson report follows in the same transaction. The previous name is kept in `SaleSalespersonEvent` together with the actor, the time, and an optional reason, and the change is written to the audit trail. A voided sale is refused.
...
Receipt photo removal has two distinct UI actions. POS and Receipt Verification offer **Remove selected photo** for an unsaved file; it clears the File/preview and native file input, permits selecting the same file again, and makes no server request. POS only shows an unsaved selection. Receipt Verification also shows the persisted photo and offers a destructive **Delete receipt photo** confirmation to `sales:evidence:delete` holders. That existing independent action grant is reused; upload permission alone does not authorize deletion. A Branch Finding replacement already uploaded while submission is awaiting retry is not treated as an unsaved removable file.

### Payment ledger

Every receipt the business issues for money received is one `Payment` row: `ORDER_DOWNPAYMENT` and `ORDER_PAYMENT` before a sale exists, `ORDER_FINAL` for the balance settled at release, and `DIRECT_SALE` for a POS sale. A receipt number is mandatory on all four and is still registered in the manual-receipt registry, so it cannot be reused.

A row linked to a sale (`ORDER_FINAL`, `DIRECT_SALE`) inherits that sale's accounting review and never appears in the payment queue; the same piece of paper is never reviewed twice. Voiding a sale voids its ledger row, and a void-and-replace moves the money onto the replacement sale's row.

`POST /api/customer-orders/:orderId/payment` now requires `reference` (the receipt number) and accepts `method`. `POST /api/customer-orders` accepts `downpaymentMethod` alongside the existing downpayment fields.

Resolving a payment mismatch as `VOIDED` restores the amount to an open order's balance and repoints its `downpaymentReceiptNumber` at the oldest remaining active payment. A completed order's payment cannot be voided; correct the sale instead. A cancelled order keeps its payments, because forfeited money was never returned to the customer.
...
`POST /api/stock-receipts` accepts `{ reference, supplierId, locationId?, notes?, lines: [{ productId, expectedQuantity, acceptedQuantity, quarantinedQuantity, missingQuantity, unitCost, claimReason?, claimNotes? }] }`. Expected quantity is positive and must equal accepted plus quarantined plus missing; split quantities are nonnegative, product IDs cannot repeat, unit cost is positive, and an affected line requires a claim reason. Every product and the Supplier must be active. Any malformed line rejects the complete request. Any holder of `inventory-receiving:create` can post without Supplier Maintenance access. The destination is the delivery's actual location: the server accepts `locationId` only when it is one of the actor's assigned active operational locations, defaults to that location when the actor has exactly one, and returns `400 LOCATION_REQUIRED` when a multi-location actor omits it. A location outside the actor's assignments returns `403 FORBIDDEN`.

Posting also notifies the receiving location's assigned users plus owner and business-wide accounts; other branches are not told. A single-product delivery links its alert to that product's inventory balance; a mixed delivery links to `/inventory?location=<code>`, which opens that branch's inventory with the location filter already applied.
...
Reports are authorized one at a time. The five grants are `reports:sales`, `reports:salesperson-sales`, `reports:inventory-summary`, `reports:stock-movement`, and `reports:returns-warranty`; a role holding one may both read that report and export its PDF, and may not touch the others. The Reports screen offers only the tabs the role holds and opens on the first of them, the sidebar entry appears when the role holds any one, and an unknown `type` is `400 UNKNOWN_REPORT` rather than a permission failure. The retired `reports:view` and `reports:export` grants are migrated by `20260921170000_per_report_permissions`, which gives every role that could previously see reports all five and drops the two old strings.

`GET /api/reports` accepts exactly `type=sales|salesperson-sales|inventory-summary|stock-movement|returns-warranty` (default `sales`). Retired `inventory-movements` and `low-stock` types, cross-report filters, unknown parameters, and duplicated parameters return `400 INVALID_FILTERS`, including PDF requests. Operational `/api/inventory/movements`, movement records, stock alerts, and dashboard low-stock views are unchanged. Old Reports page links for retired types open clean default Sales filters instead of forwarding obsolete filters.

Supported filters are: Sales and Sales by Salesperson `dateFrom`, `dateTo`, `locationId`, `salespersonId`, `source`, `paymentMethod`; Inventory Summary `locationId`, `search`, `category`, `brand`, `productStatus`; Returns & Warranty `dateFrom`, `dateTo`, `locationId`, `caseType`, `status`, `resolution`, `entitySearch`. The former Inventory `stockStatus` filter is rejected; positive availability is now mandatory. All options, rows, breakdowns, and totals remain inside effective active-location access. Both sales reports use `verifiedAt`; Returns & Warranty uses creation time as `caseDate`. `dateFrom` defaults to the first day of the current Manila month; omitted `dateTo` defaults to the last day of the supplied/default `dateFrom` month, inclusive through the next Manila midnight (exclusive). Explicit custom end dates are preserved; reversed or impossible dates are rejected.

Both sales reports share one authorized `Sale` query: `status=POSTED`, Accounting review `VERIFIED`, verification timestamp in range, and optional attribution ID, source, and payment filters. Omitted location includes all authorized active locations; a supplied ID narrows scope (literal `all` is Inventory-only). `DIRECT_SALE` selects null `orderId`, while `CUSTOMER_ORDER` selects non-null `orderId`; open orders and voided/unverified sales do not contribute. Receipt rows now include `salespersonId: string | null` alongside the stored name snapshot (`Unassigned` when absent) and separate encoder. Existing Sales detail, grand totals, branch totals, and branch percentage calculations remain unchanged.

`SalespersonSalesReport` changes the Sales discriminator to `salesperson-sales`, retains `rows`, `branchTotals`, `grandTotal`, and report metadata, and adds `salespersonTotals: [{ salespersonId: string | null, salesperson, transactionCount, units, totalDiscount, averageSale, totalAmount, percentage }]`. Groups use stable IDs, not names: same-name IDs stay distinct and null IDs form one `Not recorded (legacy)` bucket. Group labels use the newest matching sale snapshot in the applied dataset (verification timestamp descending, sale ID descending); rows retain their own snapshots. Groups sort by label then ID, not revenue/rank. Average sale is group amount divided by transaction count; percentage is group amount divided by filtered grand amount times 100, or zero for a zero grand amount. Overall totals include legacy rows and use the full dataset. UI shows overview metrics, a Salesperson summary with overall footer, and receipt tables per group with units/discount/amount subtotals; no commission, collected-payment, ranking, or Payroll semantics are added.
...
For both sales types, Salesperson options are distinct non-null IDs from posted, verified sale attributions with a non-null verification timestamp in effective scope, independent of date/source/payment filters and current personnel status, role, or home location. Inactive, reassigned, or cross-home personnel remain options only through qualifying scoped history. Labels use the newest qualifying sale snapshot, with sale ID descending breaking timestamp ties, and sort by label then ID; they can differ from a report group's date-filtered label. JSON/PDF reject a selected ID without scoped verified history with `403`; a historical option without sales in the applied period yields empty results. `/options` returns choices without performing that selected-ID membership check. Null attribution has no dedicated filter option. The four UI report controls are compact side-by-side icon/label buttons that wrap on narrow screens; navigation resets to clean report defaults rather than carrying filters between the two sales reports.
...
`GET /api/inventory` and `GET /api/inventory/movements` accept `location` as `all`, one active operational location code, or several codes separated by commas (`BL,LU`). Every code is resolved and access-checked on its own, so naming an extra branch widens nothing: a code the caller cannot reach returns `403`. Selecting no branch in the UI sends `all`.
