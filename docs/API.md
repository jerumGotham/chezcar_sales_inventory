...
Uploading receipt evidence runs free local Tesseract OCR and persists a draft containing the recognized text, confidence, detected receipt number/total, and per-line match signals. OCR never changes the Accounting review status and never verifies or corrects a sale. A failed OCR attempt preserves the private image for manual review. Receipt-list rows expose only the persisted draft/status needed by the side-by-side Accounting UI.
