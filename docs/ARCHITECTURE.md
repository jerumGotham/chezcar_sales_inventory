...
| Receipt verification | POS captures private handwritten-receipt evidence after posting; Accounting manually compares system input beside the uploaded image, with both verification and evidence-view capabilities required for review; OCR/automatic extraction is absent; Branch Finding distinguishes fresh wrong-photo re-review from an incorrectly encoded Sale awaiting authorized void and inventory reversal | `app/api/accounting/receipts/`, `lib/server/services/receipt-evidence-notifications.ts`, `lib/server/services/customer-sales.ts`, `app/pos/page.tsx`, `app/accounting/receipt-verification/` |
| Payment receipt verification | Every receipt for money received is one `Payment` ledger row; order downpayments and later order payments get their own verification queue on a second tab, with the same photo-then-confirm/report-mismatch cycle. A sale-linked row mirrors its sale's review, so one receipt is never reviewed twice. Voiding a payment returns the amount to an open order's balance | `app/api/accounting/payments/`, `lib/server/services/payments.ts`, `app/accounting/receipt-verification/payment-receipts-client.tsx` |
| Direct-sale correction requests | POS confirms complete sale details before posting; Branch may report an assigned-branch direct sale without evidence or stock effect; Admin keeps it or atomically voids and reverses the original deduction | `app/api/sales/[saleId]/correction-request/`, `lib/server/services/customer-sales.ts`, `app/customer-orders/page.tsx`, `app/accounting/receipt-verification/page.tsx` |
...
| Supplier maintenance | Global capability-delegated Supplier lifecycle plus active-only receiving lookup | `lib/server/services/suppliers.ts`, `app/api/suppliers/`, `app/suppliers/` |
| Personnel maintenance | Location-scoped non-login Salesperson/Installer master-data lifecycle | `lib/server/services/personnel.ts`, `app/api/personnel/`, `app/personnel/` |
| Personnel attribution | Narrow branch-scoped Personnel lookup; immutable Sale/Order snapshots; immutable previous/new Customer Order reassignment events; Backjob Installer eligibility rechecked at schedule, start, and completion | `app/api/customer-orders/options/route.ts`, `lib/server/services/personnel.ts`, `lib/server/services/customer-sales.ts`, `lib/server/services/backjobs.ts` |
| Quarantine-aware inventory | Per-location non-sellable bucket and shared availability invariant used by inventory, sales, orders, transfers, reports, and offline snapshots | `lib/inventory-quantity.ts`, `lib/server/catalog.ts`, `lib/server/inventory-availability.ts` |
| Returns and warranty | Location-scoped Backjob, Customer Warranty, Supplier Claim, evidence, quantity resolution, payload-checked idempotency, target-date guards, and separate physical inventory-action services | `lib/server/services/backjobs.ts`, `lib/server/services/customer-warranties.ts`, `lib/server/services/supplier-claims.ts`, `app/inventory/returns-warranty/` |
| Focused reports | Four strict report types: Sales and Sales by Salesperson share one authorized verified-sale query; Inventory Summary is current branch-only available stock; Returns/Warranty uses case dates. Dated reports default through Manila month-end. Historical Salesperson options derive from scoped verified attributions, independently of current personnel assignments; `salesperson-sales` adds stable-ID totals and grouped receipts/PDF while preserving Sales percentages and inherited PDF branch totals. Compact wrapping report controls navigate defaults; independent options follow draft scope and results/export use applied filters. Movement/Low Stock report types remain rejected without changing operational history. Source-only and unverified; no new Sales by Salesperson migration, existing pending migrations remain. See `docs/product/REPORTS-SPEC.md` | `lib/contracts/reports.ts`, `lib/server/services/reports.ts`, `lib/server/report-pdf.ts`, `app/api/reports/`, `app/reports/` |
| Stock transfers | Durable SR-to-branch transfer lifecycle with source-authorized positive-availability product options, Admin operational cover for Stock Staff actions, scoped printable all-status transfer checklists, compensating cancellation, discrepancy resolution, inventory movements, Owner audit view, and persisted notifications | `app/api/stock-transfers/`, `lib/server/services/stock-transfers.ts`, `app/stock-transfers/` |
| Stock receipts | Durable Stock Room receipts linked to active Suppliers, with accepted/quarantined/missing splits and automatic draft claims; missing-only lines create no balance, cost, or movement effect | `app/api/stock-receipts/route.ts`, `app/inventory/receive/`, `lib/server/services/stock-receipts.ts` |
...
- The sidebar renders only capability-permitted entries; prototype routes without a named central capability stay out of authenticated navigation. `/sign-in` omits the sidebar, `/pos` keeps its full-width exception, and authorized transfer checklist print routes omit the shell while retaining server-side capability and location checks.
- The Backjob print route (`/inventory/returns-warranty/[backjobId]/print`) also omits the app shell, renders no account header or module navigation, and retains its server-side `backjobs:print` and record-location authorization. Its client controls request browser printing once per mount after document load, font readiness, and a paint opportunity; manual Print, Close tab, and Back controls are screen-only. The detail link still opens a new tab. Browsers may block automatic printing or tab closing; manual Print and Back remain available. This change is source-only and has not been runtime-verified.
...
- Backjob, CustomerWarranty, SupplierClaim, their lines/events/actions/evidence/settlements, and inventory movement links
- CustomerOrderSalespersonEvent immutable reassignment audit
- Notification with per-user read state
- SaleCorrectionRequest with Branch request provenance, pending/resolved state, and Admin outcome
...
5. Inventory movements cover transfers, physically received supplier quantities, sales, Backjob parts, warranty quarantine/release, Supplier Claim dispositions, and manual adjustments; damaged/return locations and stock cards remain incomplete.
...
9. The last recorded clean baseline passed lint with 23 warnings, 204 unit tests, 83 integration tests, type-check, and build. Current uncommitted workflow/report assertions and schema changes have not been rerun.
