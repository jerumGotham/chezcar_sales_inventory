...
| Stock transfers | Durable SR-to-branch transfer lifecycle with source-authorized positive-availability product options, Admin operational cover for Stock Staff actions, scoped printable in-transit receiving checklists, compensating cancellation, discrepancy resolution, inventory movements, Owner audit view, and persisted notifications | `app/api/stock-transfers/`, `lib/server/services/stock-transfers.ts`, `app/stock-transfers/` |
