...
| Receipt verification | POS captures private handwritten-receipt evidence after posting; Accounting manually compares the system input with the original uploaded image; Branch Finding distinguishes fresh wrong-photo re-review from an incorrectly encoded Sale awaiting authorized void and inventory reversal; legacy correction responses remain supported | `app/api/accounting/receipts/`, `lib/server/services/receipt-evidence-notifications.ts`, `lib/server/services/customer-sales.ts`, `app/pos/page.tsx`, `app/accounting/receipt-verification/` |
...
| Salesperson attribution | Narrow branch-scoped Personnel lookup and immutable snapshots for Direct Sales and Customer Orders | `app/api/customer-orders/options/route.ts`, `lib/server/services/personnel.ts`, `lib/server/services/customer-sales.ts` |
...
| Returns and warranty | Location-scoped Backjob, Customer Warranty, Supplier Claim, evidence, and inventory-action services | `lib/server/services/backjobs.ts`, `lib/server/services/customer-warranties.ts`, `lib/server/services/supplier-claims.ts`, `app/inventory/returns-warranty/` |
| Focused reports | Shared validated filters and live authorized Sales, Inventory, Movement, Returns/Warranty, and Low Stock datasets/PDFs | `lib/contracts/reports.ts`, `lib/server/services/reports.ts`, `app/api/reports/route.ts`, `app/reports/` |
...
| Stock receipts | Durable Stock Room receipts linked to active Supplier master data, with historical supplier/name snapshots, SR balance increments, and inventory movements | `app/api/stock-receipts/route.ts`, `app/inventory/receive/`, `lib/server/services/stock-receipts.ts` |
...
9. Lint passes with 23 warnings; 204 unit tests, 83 integration tests, type-check, and build also pass locally.
