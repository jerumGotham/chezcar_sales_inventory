...
General Job Orders, stock-card supporting data, and parts of the offline workflow remain page-local or fixture-backed. Sales, customers, Customer Orders, the three focused reports, and the Returns/Warranty workflows documented below use persisted data; other UI presence is not evidence of persistence unless documented here or in `docs/API.md`.
...
| Focused reports | Three strict report queries: verified Sales by verification date, current branch-only available Inventory Summary, and Returns/Warranty by case date; dated reports default through Manila month-end. Independent `/api/reports/options` reads follow draft scope without applying report filters. JSON and PDF share authorization, applied scope, filters, rows, and totals; retired Movement/Low Stock report types are rejected without changing operational inventory history | `lib/contracts/reports.ts`, `lib/server/services/reports.ts`, `lib/server/report-pdf.ts`, `app/api/reports/`, `app/reports/` |
