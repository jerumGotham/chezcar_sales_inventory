...
| Stock transfers | Durable SR-to-branch transfer lifecycle with Admin operational cover for Stock Staff actions, compensating in-transit cancellation, discrepancy investigation/resolution, inventory movements, Owner audit view, and persisted workflow notifications | `app/api/stock-transfers/`, `lib/server/services/stock-transfers.ts`, `app/stock-transfers/` |
