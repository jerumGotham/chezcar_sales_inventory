...
No row below was executed during the source-only 2026-09-08 Reports revision, including Sales by Salesperson, at the user's explicit request. No tests/build/lint/typecheck/browser/PDF rendering/verification commands/migrations/generation/restart were performed for this update. The following four-report matrix is for future checks, not verification evidence. Sales by Salesperson requires no new migration; existing pending migrations and the BackjobItem database/client prerequisite remain pending. See `docs/MANUAL-TEST-FLOWS.md` for detailed attribution and grouping cases.
...
| Sales by Salesperson (`salesperson-sales`) | Same filters, Manila verification dates, draft/Apply/Reset behavior, and authorized verified-sale query as Sales | Stable-ID groups, distinct same-name IDs, newest matching snapshot label, one null-ID legacy bucket; grouped receipts/subtotals reconcile to unchanged Sales grand totals and percentages | Both sales selectors retain inactive/reassigned/cross-home attributions only through scoped verified history; out-of-scope IDs denied; PDF includes Salesperson and inherited branch subtotals, overall totals, grouped receipt details; no commission/ranking |
| Inventory Summary | Authorized active branch (default one; explicit `all` compares), product search, category, brand, product status; current snapshot, no stock-status filter | One row per positive-availability product; product metadata and available/per-branch columns; 25 per UI page, full filtered totals/PDF; missing/nonpositive branch contributions zero | Stock Room absent from selectors/rows/totals; explicit warehouse request denied; warehouse-only actor gets empty scope; PDF parity |
...
Check compact side-by-side report buttons, wrapping/active state on narrow screens, and default-reset navigation between all four reports. Salesperson grouping/options, responsive/light/dark behavior, and grouped PDF layout remain source-only and unverified.
...
Better Auth sessions, active-account checks, persisted capability authorization, session revocation, the internal credential surface, and the first-login prompt now have unit and integration suites (see the checked-in suites above). Extend this matrix as each new protected page, route handler, and mutation is implemented: include unauthenticated, inactive/expired-session, missing-grant, wrong-scope, revoked-session, and allowed cases.
